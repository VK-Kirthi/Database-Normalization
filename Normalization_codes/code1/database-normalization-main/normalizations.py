...
def is_1nf(relation):
    """
    Check if the relation is already in 1NF.
    Returns True if it is, False otherwise.
    """
    if relation.empty:
        return True  # An empty relation is trivially in 1NF.

    for column in relation.columns:
        # Ensure no non-atomic values (list, set, dict) in the column
        if relation[column].apply(lambda x: isinstance(x, (list, set, dict))).any():
            return False
        
        # Ensure all values are of the same type in the column
        if relation[column].apply(type).nunique() > 1:
            return False
...
def first_normalization_form(relation, primary_key):
    """
    Convert the given relation to 1NF.
    - relation: DataFrame with the input table data
    - primary_key: Tuple containing primary key column names
    Returns: A dictionary with the normalized DataFrame and a boolean flag indicating if it was already in 1NF.
    """
    # Dictionary to store the 1NF relation
    relations = {}
    one_flag = is_1nf(relation)

    # If already in 1NF, return the relation with flag as True
    if one_flag:
        print("The relation is already in 1NF.")
        relations[primary_key] = relation
        return relations, one_flag

    # If normalization is needed, set one_flag to False and perform the normalization
    one_flag = False
    for col in relation.columns:
        if relation[col].apply(lambda x: isinstance(x, (list, set))).any():
            relation[col] = relation[col].apply(lambda x: list(x) if isinstance(x, set) else x)
            relation = relation.explode(col, ignore_index=True)
    one_flag = is_1nf(relation)
    if(one_flag):
