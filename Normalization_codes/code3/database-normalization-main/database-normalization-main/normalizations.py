...
def fourth_normalization_form(relations, mvd_dependencies):
    ...
    if four_flag:
        print('RELATIONS AFTER 4NF (Already in 4NF):')
        for relation_name, relation in relations.items():
    ...
    print('Decomposed relations after 4NF:')
    for rel in four_relations:
        print(f"{rel}:")
        print(four_relations[rel])
        print('\n')
...
def fifth_normalization_form(relations, primary_key, dependencies):
    five_relations = {}
    five_flag, candidate_keys_dict = is_5nf(relations)

    if five_flag:
        print('RELATIONS AFTER 5NF (Already in 5NF):')
        for relation_name, relation in relations.items():
            print(f"{relation_name}:")
            print(relation)
            print('\n')
        ...
        for relation_name, relation in relations.items():
            candidate_keys = candidate_keys_dict[relation_name]
            decomposed_relations = decompose_5nf(relation_name, relation, candidate_keys)

            for decomposed_relation in decomposed_relations:
                five_relations[relation_name] = decomposed_relation  # Store each decomposed relation

            print(f"Decomposed relations for {relation_name}:")
...
def dknf_normalization_form(relation, primary_key, dependencies, domain_constraints):
    dknf_tables = []
    dknf_flag = True  # Assume it satisfies DKNF unless proven otherwise

    # If domain_constraints is empty, we skip domain constraint checks
    if not domain_constraints:
        print("No domain constraints provided; proceeding without domain checks.")

    # Process dependencies for DKNF
    for det, dep in dependencies.items():
        # Ensure 'det' is a tuple for hashing purposes
        det = tuple(det)  # Convert det to a tuple to make it hashable

        # Prepare the list of columns: combine det and dep
        column_list = list(det) + dep  # Ensure you're combining as lists here

        # Check if all columns in column_list exist in the relation
        if all(col in relation.columns for col in column_list):
            new_table = relation[column_list].drop_duplicates()  # Normalize based on det and dep
            dknf_tables.append(new_table)
        else:
            print(f"Columns {column_list} do not exist in the relation.")

    # Additional checks with domain constraints, if they exist
    for attribute, constraint in domain_constraints.items():
        if attribute in relation.columns:
            # Check the attribute values in relation
            if not relation[attribute].astype(str).str.contains(constraint).all():
                dknf_flag = False
                print(f"Domain constraint violated for {attribute}: expected {constraint}")

    print('RELATIONS AFTER DKNF:')
    for i, table in enumerate(dknf_tables):
        print(f"Table {i + 1}:")
        print(table)
        print('\n')

    return dknf_tables, dknf_flag
